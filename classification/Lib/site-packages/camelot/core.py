...
class Table(object):
    ...
    def to_txt(self, **kwargs):
        ...
        # text = self.df.to_csv(**rawtxt)
        rawtxt = {"encoding": "utf-8", "index": False, "header": False, "quoting": 1}
        rawtxt.update(kwargs)
        text = self.df.to_csv(**rawtxt)
        return(text)
...
class TableList(object):
    ...
    def export(self, path, f="csv", compress=False):
        ...
        if f in ["csv", "html", "json", "markdown"]:
            ...
            for table in self._tables:
                sheet_name = f"page-{table.page+1}-table-{table.order}"
            ...
            for table in self._tables:
                text = table.to_txt()
                ftext = text.encode('ascii', 'replace').decode()
                if len(ftext.replace('"', '')) < 200:
                    continue
                    #print(f'{table.page} {table.order} EMPTY TABLE? PRINTING OUTPUT HERE: // {len(ftext)} //')
                metadata = f'"===Page Number: {table.page}=== ---Table Number: {table.order}---"'
                tables.append(f'{metadata}\n\n{ftext}\n"=============="\n\n\n')
            final_table = ''.join(tables)
