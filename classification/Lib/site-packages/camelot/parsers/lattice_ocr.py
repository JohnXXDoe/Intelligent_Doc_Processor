...
class LatticeOCR(BaseParser):
    def __init__(
        ...
        iterations=0,
        resolution=300,
        #ReadTxt arguments from easyocr
        ycenter_ths=0.5,
        height_ths=0.5,
        width_ths=0.05,
        **kwargs,
    ...
    def _generate_image(self):
        ...
        page = convert_from_path(self.filename, self.resolution, poppler_path=r"C:\poppler-0.68.0\bin", timeout=10000)
        page[0].save(self.imagename)
        ...
        for r_idx in range(len(table.cells)):
            for c_idx in range(len(table.cells[r_idx])):
                ...
                with TemporaryDirectory() as tempdir:
                    ...
                    text = self.reader.readtext(temp_image_path, detail=0, ycenter_ths=0.5, height_ths=0.5, width_ths=0.05)
                    text = " ".join(text)
    ...
    DI-WHY
    '''
    def _generate_text(self, table_idx, cols, rows, **kwargs):
        table = Table(cols, rows)
        # set table edges to True using ver+hor lines
        table = table.set_edges(self.vertical_segments, self.horizontal_segments, joint_tol=self.joint_tol)
        # set table border edges to True
        ...
        for r_idx in range(len(table.cells)):
            for c_idx in range(len(table.cells[r_idx])):
                ...
                table.cells[r_idx][c_idx].text = text
        table = LatticeOCR._copy_spanning_text(table, copy_text={'h', 'v'})
        data = table.data
        #print(data)
        return data
    ...
    def extract_tables(self, filename, suppress_stdout=False, layout_kwargs={}):
        ...
        for table_idx, tk in enumerate(
            ...
            txt_table = self._generate_text(table_idx, cols, rows)
            table._bbox = tk
            _tables.append(table)
            txt_tables.append(txt_table)

        return _tables, txt_tables